    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                results, maps, times = test.test(data=opt.data,  # 确保传入data参数
                                      weights=opt.weights,  # 传入weights参数
                                      batch_size=total_batch_size,
                                      imgsz=imgsz_test,
                                      conf_thres=0.001,  # 可根据需要调整
                                      iou_thres=0.6,  # 可根据需要调整
                                      model=ema.ema,
                                      single_cls=opt.single_cls,
                                      dataloader=testloader,
                                      save_dir=log_dir,
                                      save_txt=False,  # 可根据需要调整
                                      save_conf=False,  # 可根据需要调整
...
if __name__ == '__main__':
    ...
    if opt.resume:  # resume an interrupted run
        ...
        with open(log_dir / 'opt.yaml', 'r', encoding='utf-8') as f:
